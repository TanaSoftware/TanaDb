...

{
    public class MailSender
    {
        public static bool sendMail(string subject, string content, string[] recipients, string from)
        {
            ...
            //string contentHtml = BuildMessageBody(content);
            
            byte[] bytes = Encoding.Default.GetBytes(content);
            string EncContent = Encoding.UTF8.GetString(bytes);

            byte[] bytes2 = Encoding.Default.GetBytes(subject);
            string EncSubject = Encoding.UTF8.GetString(bytes2);

            using (var msg = new System.Net.Mail.MailMessage(from, recipients[0], EncSubject, EncContent))
            {
