...

{
...

    {
...

        {
            ...
            //}
            //האם קיים לקוח שבעל עסק הקים
            int customerId = 0;            
            
            //צריך לעדכן את הרשומה
            string sql = "select [Id],[guid] From Customer where [tel]=@tel";
            DataBaseRetriever db = new DataBaseRetriever(ConfigManager.ConnectionString);
            IEnumerable<CustomerObj> CustomerDetails = db.QueryData<CustomerObj>(sql, 1, new { tel = customer.tel + "@" });
            foreach (CustomerObj cust in CustomerDetails)
            {
                customerId = cust.Id;
                
                customer.guid = cust.guid;
            }
            if (customerId > 0)
        ...
        private bool sendCustomerMail(CustomerObj user)
        {
            ...
            arr[0] = user.Email;
            return MailSender.sendMail("הרשמה למערכת תורים", content, arr, "info@tori.co.il");
...
(@Name,@Email,@Password,@tel,@guid,@City);";
        ...
        public object UserAddHisCustomer(AddedCustomerByUser customer)

        {
...

            {
                ...
                return new { err="נא להזין שם", data = "" };
            ...
            {
                return new { err = "נא להזין טלפון", data = "" };
            ...
            {
                return new { err = "אין באפשרותך להוסיף משתמש", data = "" };                
            ...
            bool isAddCustomer = isMailExists || isTelExists;
            int customerId = 0;
            ...
            if (!isAddCustomer)
            {
                ...
                {
                    return new { err = "ארעה שגיאה", data = "" };                    
            ...
            else
            {
                ...
                string sql = "select [Id] From Customer where [tel]=@tel";
                DataBaseRetriever db = new DataBaseRetriever(ConfigManager.ConnectionString);
                IEnumerable<int> CustomerId = db.QueryData<int>(sql, 1, new { mail = user.Email, tel = user.tel });                
                ...
                if (customerId > 0)
                {
                    ...
                    try
                    {
                        var Rows = db.Execute(sqlUserCustomerInsert, 1, new { UserId = customer.Id, CustomerId = customerId });
                    }
                    catch
                    {
            ...
            //                MailSender.sendMail("הרשמה למערכת תורים", content, arr, "info@tori.co.il");
            //                break;
            //            }
            //        }
            //    }
            ...
            //}
            if (customerId > 0)
                return  new { err = "", data = customerId.ToString() }; 
        ...
        public bool ActivateCustomer(string guid)
...
(@User,@Email,@Password,@BizName,@tel,@BizNameEng,@Adrress,@City,@Country,@CreditCard,@guid,@IsGroup);";
        ...
        public string DeleteEmployee(BizTypeObj user)
        {
            try
            {
                ...
                string sql2 = "Delete From EmployeesActivities where [UserId]=@UserId And [EmployeeId]=@EmployeeId";
                var affectedRows2 = db.Execute(sql2, 1, new { EmployeeName = user.EmployeeName, UserId = user.UserId, EmployeeId = user.EmployeeId });
        ...
        public string DeleteUserActivity(UserActivity userActivity)
        {
            ...
            try
            {
                ...
                string sqlAct2 = "DELETE FROM EmployeesActivities Where ActivityId=@UserId;";
                var affectedRows2 = db.Execute(sqlAct2, 1, userActivity);
            }
            catch(Exception ex)
            {
                Logger.Write(ex);
        ...
        public string ResetPasswordPhase1(string mail)
        {
            ...
            string[] arr = new string[1];
            arr[0] = mail;
            bool isOk = MailSender.sendMail("מערכת תורים", content, arr, "info@tori.co.il");
            if (!isOk)
                return "ארעה שגיאה";
        ...
        public IEnumerable<UserActivity> GetBizForCustmer(UserObj user)
        {
            if (!IsCustomerGuidExists(user.guid))
                return null;

            string sql = "SELECT [Id] as UserId,[User] as Name FROM Users WHERE City=@City And [Active] is not null ;";

            DataBaseRetriever db = new DataBaseRetriever(ConfigManager.ConnectionString);

            IEnumerable<UserActivity> users = db.QueryData<UserActivity>(sql, 1, new { City = user.City });
            return users;
        }

        public string UpdateUserColor(string guid, int id, string color)
        {
            ...
            string sql = "Update [UsersActivitiesTypes] set [color]=@color where [Id]=@id";

            DataBaseRetriever db = new DataBaseRetriever(ConfigManager.ConnectionString);

            var affectedRows = db.Execute(sql, 1, new { id = id, color = color });
